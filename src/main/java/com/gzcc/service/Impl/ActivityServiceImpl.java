...
//            BeanUtils.copyProperties(activityRepository.findAll(example,pageable).getContent(),activityVOList);
     ...
     */
    @Override
    public ActivityListVO getAllReportedActivity(int nowPage,int pageSize,String studentId,Short status){


//        try{
//            StudentActivities studentActivity = new StudentActivities();
//            Short[] arrays = new Short[]{0,1,2,3};
//            List<Short> creditTypeList = Arrays.asList(arrays);
//            List<Short> collect = creditTypeList.stream().filter(type -> type.equals(status)).collect(Collectors.toList());
//            studentActivity.setStatus(collect.get(0));
//            ExampleMatcher exampleMatcher = ExampleMatcher.matching().withMatcher("status",ExampleMatcher.GenericPropertyMatchers.exact());
//            Example<StudentActivities> example = Example.of(studentActivity,exampleMatcher);
//            PageRequest pageable = new PageRequest(nowPage,pageSize);
...
//        }
        ...
        try{
            ...
            for (int i = 0;i < activities.size();i++){
                StudentActivities byStudentIdAndActivityId = studentActivitiesRepository
                        .findByStudentIdAndActivityId(studentId, activities.get(i).getUid());
                if((byStudentIdAndActivityId != null)&&(status == studentActivitiesRepository.findByStatus(activities.get(i).getUid()))){
                    activities2.add(activities.get(i));
                }
            }
            ActivityListVO activityListVO = new ActivityListVO();
//            PageRequest pageable = new PageRequest(nowPage,pageSize);
//            Page<Activity> page = (Page<Activity>)activities2;
//
//            activityListVO.setCount((int) page.getTotalElements());//总记录数
//            activityListVO.setCurrent(page.getNumber());//当前页
//            PageRequest tempPageable = new PageRequest(page.getTotalPages(),Const.pageSize);
...
//            activityListVO.setPrevious(Const.pagePre+prePage);//上一页
     ...
     */
    @Override
    public String sginIn(String studentId,String activityUid){

        final StudentActivities byStudentIdAndActivityId = studentActivitiesRepository.findByStudentIdAndActivityId(studentId, activityUid);
        if(byStudentIdAndActivityId != null){
            byStudentIdAndActivityId.setStatus((short)2);
            studentActivitiesRepository.save(byStudentIdAndActivityId);
            return "success";
        }
        return "fail";
    }
